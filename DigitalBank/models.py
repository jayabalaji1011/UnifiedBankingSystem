...
class Bank(models.Model):
    ...
    bank_prefix = models.CharField(max_length=4, default='1011')  # first 4 digits for account number
...
class Customer(models.Model):
    ...
    bank = models.ForeignKey(Bank, on_delete=models.SET_NULL, null=True, blank=True)
    aadhar = models.ForeignKey(Aadhar, on_delete=models.PROTECT, related_name='digitalbank_aadhar_customers')
    pan = models.ForeignKey(Pan, on_delete=models.PROTECT, related_name='digitalbank_pan_customers', null=True, blank=True)
    ...
    photo = models.ImageField(upload_to='digitalbank_photos/', blank=True, null=True)
    ...
    # Default password: last 6 digits of mobile + dob in ddmmyy
...
class Transaction(models.Model):
    TRANSACTION_TYPE = [
        ...
        ('WITHDRAW', 'Withdraw'),
        ('TRANSFER', 'Transfer'),
        ('CREDIT', 'Credit'),  # receiver via UPI / interbank credit
        ('DEBIT', 'Debit'),    # sender via UPI / interbank debit
    ]
