...
def staff_login(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return redirect('digitalbank:staff_dashboard')
        except Staff.DoesNotExist:
            messages.error(request, "User not found!")
    return render(request, "digitalbank/staff_login.html", {'form': form})
...
def logout_staff(request):
    ...
    return redirect('digitalbank:staff_login')
...
def staff_account(request): 
    ...
    if request.method == 'POST':
        ...
        return redirect('digitalbank:staff_account')

    return render(request, 'digitalbank/staff_account.html', {'staff': staff})
...
def staff_dashboard(request):
    ...
    if query:
        try:
            ...
            return redirect('digitalbank:customer_detail', pk=customer.customer_id)
    ...
    return render(
        request,
        "digitalbank/staff_dashboard.html",
...
def bank_dashboard(request):
    ...
    return render(request, 'digitalbank/bank.html', context)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                return render(request, 'digitalbank/create_start.html', {'form': form, 'error': 'Aadhar+mobile mismatch or not found.'})
            ...
            # session store
            request.session['digitalbank:customer_pending_aadhar_id'] = a.pk
            request.session['digitalbank:customer_pending_mobile'] = mobile
            # Dev show OTP
            return render(request, 'digitalbank/verify_otp.html', {'aadhar': a, 'otp': otp, 'pan_exists': pan_exists})
    else:
        form = CustomerCreateStartForm()
    return render(request, 'digitalbank/create_start.html', {'form': form})
...
def customer_verify_otp(request):
    ...
    try:
        ...
        return render(request, 'digitalbank/verify_otp.html', {'error':'Aadhar not found.'})
    try:
        entry = AadharOTP.objects.filter(aadhar=a, mobile=mobile, otp=otp, used=False).latest('created_at')
    except AadharOTP.DoesNotExist:
        return render(request, 'digitalbank/verify_otp.html', {'aadhar': a, 'error':'Invalid OTP.'})
    if entry.is_expired():
        return render(request, 'digitalbank/verify_otp.html', {'aadhar': a, 'error':'OTP expired.'})
    entry.used = True
    entry.save()
    request.session['customer_verified_aadhar_id'] = a.pk
    return redirect('digitalbank:customer_create_form')

def customer_create_form(request):
    verified_id = request.session.get('customer_verified_aadhar_id')
    if not verified_id:
        return redirect('digitalbank:create_customer_start')
...
# ✅ FALLBACK → COPY PHOTO FROM AADHAR
            ...
            return redirect('digitalbank:customer_detail', pk=cust.customer_id)
    else:
    ...
    return render(request, 'digitalbank/create_form.html', {'form': form, 'aadhar': a})
...
def create_atm(request, pk):
    ...
    if not cust.is_active:
        ...
        return redirect("digitalbank:customer_detail", pk=pk)
...
def atm_process_page(request, pk, action):
    cust = get_object_or_404(Customer, pk=pk)
    return render(request, "digitalbank/atm_processing.html", {
        "customer": cust,
        "action": action
    })
...
def customer_detail(request, pk):
    bank = Bank.objects.get(name="DigitalBank")
    ...
    return render(request, 'digitalbank/customer_detail.html', {
        'customer': cust,
        'transactions': transactions,
        'form': form,
...
def customer_edit(request, pk, bank_name):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("digitalbank:customer_detail", pk=updated.pk)
    ...
    return render(request, "digitalbank/customer_edit.html", {
        "form": form,
        "customer": cust
    })
...
def create_transaction(request, customer_id):
    ...
    if not cust.is_active:
        messages.error(request, "Customer account is deactivated")
        return redirect("digitalbank:customer_detail", pk=customer_id)
    ...
    return render(request, 'digitalbank/create_transaction.html', {'form': form, 'customer': cust})
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                return render(request, "digitalbank/atm_home.html", {'form': form})
            ...
            return redirect("digitalbank:pin_option")
...
def pin_option(request):
    ...
    if not ok:
        messages.error(request, msg)
        return redirect("digitalbank:atm_home")

    return render(request, "digitalbank/atm_options.html", {"card": card})
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if mobile != card.customer.mobile:
                ...
                return redirect("digitalbank:atm_request_otp")
            ...
            return redirect("digitalbank:atm_enter_otp")

    return render(request, "digitalbank/atm_mobile.html", {"form": form})




# Step 4: Enter OTP
from .bank_otp_service import verify_bank_otp
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                return render(request, "digitalbank/atm_otp.html", {"form": form})
            ...
            return redirect("digitalbank:atm_set_pin")
...
def atm_set_pin(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("digitalbank:atm_pin_result")


    return render(request, "digitalbank/atm_set_pin.html", {"form": form})
...
def atm_pin_result(request):
    ...
    if status == "success":
        return render(request, "digitalbank/atm_pin_success.html")
    else:
        return render(request, "digitalbank/atm_pin_failed.html")
